...
$logDir = Join-Path $PSScriptRoot 'phase_locked_logs_20260811'
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$pending = [System.Collections.Generic.Queue[int]]::new()
foreach ($id in $TaskIndices) {
    if ($id -lt 1 -or $id -gt 30) { throw "Bad task $id" }
    $pending.Enqueue($id)
}
$running = @()
$done = @()
while ($pending.Count -gt 0 -or $running.Count -gt 0) {
    while ($pending.Count -gt 0 -and $running.Count -lt $MaxParallel) {
        ...
        $batch = "PHASE_LOCKED_INDEX=$id; run(fullfile(pwd,'Figure 3','run_phase_channel_locked_pair_20260811.m'));"
...
Write-Output "PHASE_LOCKED_PARALLEL_PASS tasks=$($done.Count)"
