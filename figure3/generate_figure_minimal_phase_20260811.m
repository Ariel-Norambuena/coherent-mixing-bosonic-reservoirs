...
fig=figure('Color','w','Visible','off','Units','inches','Position',[1 1 7.5 6.85]);
...
ax4=nexttile;hold(ax4,'on');box(ax4,'on');grid(ax4,'on');values=[fullIQ.fullIQLinearNRMSE fullIQ.fullIQNVAR2NRMSE phase.nvar2NRMSE phase.J0NRMSE phase.coupledNRMSE];methodColors=[gray;.15 .58 .58;purple;blue;red];for m=1:5,scatter(ax4,m+.08*linspace(-1,1,height(phase)),values(:,m),14,methodColors(m,:),'filled','MarkerFaceAlpha',.38);plot(ax4,[m-.20 m+.20],mean(values(:,m))*[1 1],'-','Color',methodColors(m,:),'LineWidth',1.8);end;xticks(ax4,1:5);xticklabels(ax4,{'I/Q lin.','I/Q quad.','$\phi$ quad.','$\phi$, $J=0$','$\phi$, $J=.65$'});set(ax4,'TickLabelInterpreter','latex');xtickangle(ax4,30);ylabel(ax4,'test NRMSE');title(ax4,'(d) Phase-stream benchmark','FontWeight','normal');ylim(ax4,[.075 .29]);
